...
SELECT Vacant, Dormant FROM
    (SELECT DISTINCT A.StartStationID as 'Vacant', B.EndStationID 'Dormant' From dbo.CitiBike A, dbo.CitiBike B
        WHERE A.StartStationId NOT IN (SELECT EndStationID FROM dbo.CitiBike) AND B.EndStationID NOT IN (SELECT StartStationID FROM dbo.CitiBike)) as VacantAndDormant
